...
def process_csv_file(file_path):
    ...
    # Note: We collect all unique labels first to ensure 'bckg' is handled correctly
    ...
    if not seizure_labels_in_file: # 如果文件中没有任何癫痫标签，则此文件不满足“含有seizure label”的条件
        # print(f"No seizure labels (non-bckg) found in {file_path}. Skipping.")
        return None, None
    ...
    for channel_val in all_channels:
        ...
        if not has_seizure_label:
            # 进一步确认，确保它至少有'bckg'标签 (防止空标签的情况，尽管根据数据应该不会发生)
    ...
    if seizure_labels_in_file and bckg_only_channels:
        return {
            'bname': bname,
            'seizure_type': ', '.join(seizure_labels_in_file),
...
def main():
    ...
    all_bckg_channels_overall = [] # 存储所有文件中符合条件的bckg only通道
    ...
    for dirpath, dirnames, filenames in os.walk(root_directory):
        for filename in filenames:
            if filename.endswith('.csv'):
                ...
                file_info, bckg_channels_from_current_file = process_csv_file(file_path)
                
                if file_info: # 如果文件被成功处理并找到了符合条件的数据
                    processed_files_info.append(file_info)
                    all_bckg_channels_overall.extend(bckg_channels_from_current_file)
    ...
    # Count the occurrences of different 'bckg' channels across all processed files
    if all_bckg_channels_overall:
        channel_counts = pd.Series(all_bckg_channels_overall).value_counts()

        print("\nChannel counts (only bckg in seizure files across all dataset):")
        print(channel_counts)

        # Plot the bar chart
        plt.figure(figsize=(12, 7))
        channel_counts.plot(kind='bar')
        plt.title('Count of Background Channels in Seizure-Labeled Files (Across all CSVs)')
        plt.xlabel('Channel Name')
        plt.ylabel('Count')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig('total_background_channel_counts.png')
        print("\nBar chart saved as 'total_background_channel_counts.png'")
    else:
